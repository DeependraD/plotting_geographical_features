...
## read districtwise wheat area profile
dist_wise_wheat <- readxl::read_xlsx("./data/npl_wheat_districts_0506-1516.xlsx", "area", skip = 1)
...
wheat_area_1516 <- ggplot() +
  ...
  annotate(geom = "table", x = 690000, y = 3380000, label = list(np_dist %>%
                                                                   ...
                                                                   select(DISTRICT, "Area (Ha)" = Year201516)),
           vjust = 1, hjust = 0) +
  scale_y_continuous() +
  ...
  xlab(NULL) + ylab(NULL) +
  theme_bw() + 
  ggtitle("Estimated districtwise area under Wheat grain in Nepal, featuring top 10 districts during\nYear: 2015/16\n(Source: Statistical Year Book, 2017)") +
...
ggsave("./outputs/npl_top_wheat_growing_districts_areawise.png", 
       plot = wheat_area_1516, units = "in", dpi = 300, width = 10)
