...
dist_wise_rice <- readxl::read_xlsx("./data/npl_rice_districts_0506-1516.xlsx", sheet = "production", skip = 1)
...
rice_production_1516 <- ggplot() +
  ...
  xlab(NULL) + ylab(NULL) +
  theme_bw() + 
  ggtitle("Estimated districtwise production of rice grain in Nepal, featuring top 10 districts during\nYear: 2015/16\n(Source: Statistical Year Book, 2017)") +
...
ggsave("./outputs/npl_top_rice_growing_districts_productionwise.png", 
       plot = rice_production_1516, units = "in", dpi = 300, width = 12, height = 8)
