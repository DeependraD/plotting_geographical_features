...
eight_dist <- c("CHITWAN", "MORANG", "JHAPA", "KABHREPALANCHOK", "BHAKTAPUR", "RUPANDEHI", "LAMJUNG", "DANG")

# plotting only selected features
plot(st_geometry(np_dist[np_dist$DISTRICT %in% eight_dist, ]))
...
# plot feature geometry with centroid
plot(sf::st_geometry(np_dist), 
     # col = sf::sf.colors(5, categorical = T), 
     border = "black", axes = F)
plot(sf::st_geometry(sf::st_centroid(np_dist)), pch = 4, col = "red", add = TRUE)
...
plot(sf::st_geometry(sf::st_centroid(np_dist[np_dist$DISTRICT %in% eight_dist, ])), 
...
eight_gg <- ggplot() +
  ...
  geom_sf(data = st_geometry(np_dist[np_dist$DISTRICT %in% eight_dist, ]), 
          aes(fill = eight_dist), lwd = 0.3) +
  geom_sf(data = st_geometry(sf::st_centroid(np_dist[np_dist$DISTRICT %in% eight_dist, ]))) +
  ggrepel::geom_text_repel(data=(np_dist[np_dist$DISTRICT %in% eight_dist, ]) %>% 
...
eight_gg
ggsave("./outputs/selected_eight.png", 
